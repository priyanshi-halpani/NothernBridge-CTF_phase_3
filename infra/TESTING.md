...
| Actual | `-rw-r----- 1 root www-data 321`. Body: "NoT in DB / Git" wording; token for this deployment **`NCC{e21fec7104b8}`** (the previous deployment produced a different token, `NCC{220a4dbc1ee6}` → per-deployment uniqueness confirmed) |
| Result | ✔ PASS |
